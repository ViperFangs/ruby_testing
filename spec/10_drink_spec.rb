...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_boba) { described_class.new('boba', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_boba.type).to eq('boba')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_boba).to be_full
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_large_boba) { described_class.new('boba', 24) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_large_boba).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_small_boba) { described_class.new('boba', 12) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_small_boba).not_to be_full
      end
    end
  end
end
